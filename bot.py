...
def new(bot, update):
    ...
    if game and game.state == GameStatus.waiting:
        ...
        bot.sendMessage(
            update.message.chat_id,
            'And we are already playing 😁 To finish the current game, use the command /cancel')
    else:
    ...
    if not game:
        ...
        if game.game_master.identity != update.message.from_user.id:
            ...
            if len(game.players) == 0:
                bot.sendMessage(update.message.chat_id, 'To play mafia you need players 😊')
                return
...
def cancel(bot, update):
    ...
    if game:
        ...
        if game_master.identity != update.message.from_user.id:
            ...
            bot.sendMessage(update.message.chat_id, 'Game paused 😐')
    else:
